...
**Error Responses:**
- `{"message":"No Cameras Enabled for Motion","code":304}` - At least one camera must have motion detection enabled before the system can be disarmed. This error may occur if all cameras have motion detection disabled.
...
curl --request POST \
  --url "https://rest-{region}.immedia-semi.com/api/v1/accounts/{AccountID}/networks/{NetworkID}/state/disarm" \
...
curl --request POST \
  --url "https://rest-${HOST}/api/v1/accounts/${ACCOUNT_ID}/networks/${NETWORK_ID}/state/disarm" \
  --header "Authorization: Bearer $NEW_TOKEN" \
  --header "Content-Type: application/json"
```
